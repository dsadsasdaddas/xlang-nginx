...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    int32_t fd = __xlang_tcp_listen(8080);
    int32_t client = accept(fd, 0, 0);
    const char * req = __xlang_recv_str(client);
    const char * body = "hello from xlang";
    const char * header = __xlang_str_concat(__xlang_str_concat("HTTP/1.1 200 OK\r\nContent-Length: ", __xlang_int_to_str((int32_t)strlen(body))), "\r\n\r\n");
    const char * resp = __xlang_str_concat(header, body);
