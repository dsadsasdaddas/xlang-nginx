...
const char * parse_path(const char * req);
int32_t serve(int32_t fd, const char * full, const char * path);
int32_t main(int argc, char** argv);

const char * mime_of(const char * path) {
    if ((__xlang_str_find(path, ".html") >= 0)) {
        return "text/html";
    }
    if ((__xlang_str_find(path, ".htm") >= 0)) {
        return "text/html";
    }
    if ((__xlang_str_find(path, ".css") >= 0)) {
        return "text/css";
    }
    if ((__xlang_str_find(path, ".js") >= 0)) {
        return "application/javascript";
    }
    if ((__xlang_str_find(path, ".json") >= 0)) {
        return "application/json";
    }
    if ((__xlang_str_find(path, ".txt") >= 0)) {
        return "text/plain";
    }
...
const char * parse_path(const char * req) {
    int32_t sp1 = __xlang_str_find(req, " ");
    if ((sp1 < 0)) {
        return "/index.html";
    }
    const char * rest = __xlang_str_slice(req, (sp1 + 1), (int32_t)strlen(req));
    int32_t sp2 = __xlang_str_find(rest, " ");
    const char * path = "";
    if ((sp2 < 0)) {
        (path = rest);
    } else {
        (path = __xlang_str_slice(rest, 0, sp2));
    }
    if ((strcmp(path, "/") == 0)) {
        (path = "/index.html");
    }
...
int32_t serve(int32_t fd, const char * full, const char * path) {
    if (__xlang_file_exists(full)) {
        int32_t size = __xlang_file_size(full);
        const char * mime = mime_of(path);
        __xlang_sb_new();
        __xlang_sb_push("HTTP/1.1 200 OK\r\n");
        __xlang_sb_push("Content-Type: ");
        __xlang_sb_push(mime);
        __xlang_sb_push("\r\nContent-Length: ");
        __xlang_sb_push(__xlang_int_to_str(size));
        __xlang_sb_push("\r\nConnection: keep-alive\r\n\r\n");
        send(fd, __xlang_sb_str(), strlen(__xlang_sb_str()), 0);
        int32_t ffd = __xlang_open_read(full);
        __xlang_sendfile_fd(fd, ffd, size);
        close(ffd);
    } else {
...
int32_t main(int argc, char** argv) {
    __xlang_argc_g = argc;
    __xlang_argv_g = argv;
    const char * docroot = "webroot";
    if (((__xlang_argc_g) >= 2)) {
        (docroot = __xlang_argv_g[1]);
    }
    int32_t listen_fd = __xlang_tcp_listen(28082);
    __xlang_set_nonblock(listen_fd);
    __xlang_epoll_create();
    __xlang_epoll_add(listen_fd);
    while (true) {
        int32_t fd = __xlang_epoll_wait((-1));
        if ((fd == listen_fd)) {
            while (true) {
                int32_t client = accept(listen_fd, 0, 0);
                if ((client < 0)) {
                    break;
                }
                __xlang_set_nonblock(client);
                __xlang_set_nodelay(client);
                __xlang_epoll_add(client);
            }
        } else {
            const char * req = __xlang_recv_str(fd);
            if (((int32_t)strlen(req) == 0)) {
                __xlang_epoll_del(fd);
                close(fd);
            } else {
                const char * path = parse_path(req);
                const char * full = __xlang_str_concat(docroot, path);
                serve(fd, full, path);
            }
        }
    }
