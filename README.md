...
- `server_proxy.x` — **reverse proxy** (nginx `proxy_pass`): prefork workers, forwards to an upstream and relays the response (binary-safe, Content-Length framed). Each worker reuses one persistent upstream connection (keepalive) — matches/beats the backend at c≥16. 9-case suite in `bench/proxy_test.sh`.
